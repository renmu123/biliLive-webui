import { z as cB, A as cM, d as defineComponent, D as useConfig, H as useTheme, p as computed, J as useThemeClass, v as h, dx as typographyLight, aU as createKey, o as openBlock, ah as createElementBlock, e as createBaseVNode, aj as mergeModels, ak as useModel, r as ref, a8 as Fragment, an as renderList, a as createVNode, w as withCtx, ap as toDisplayString, cr as renderSlot, ao as normalizeClass, b as unref, av as normalizeStyle, d4 as withModifiers, f as createTextVNode } from "./index-BdK_bIY-.js";
import { k as useDropZone } from "./index-Dx5QtryB.js";
import { s as showDirectoryDialog } from "./showDirectoryDialog-DEycIJ5C.js";
import { b as useCompitable, N as NIcon } from "./index-C4u8aLk_.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const handleFileSelect = async () => {
      ...
      let items = files.map(window.api.formatFile).filter((file) => !fileList.value.map((item) => item.path).includes(file.path));
      if (props.max) {
    ...
    function onDrop(files) {
      ...
      if (files) {
        let items = Array.from(files).map((file) => window.api.formatFile(window.api.common.getPathForFile(file))).filter((file) => !fileList.value.map((item) => item.path).includes(file.path)).filter((file) => {
...
const __unplugin_components_1 = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-8f272bbc"]]);
