import { d as defineComponent, al as mergeModels, am as useModel, r as ref, x as computed, c as createElementBlock, at as normalizeStyle, ap as normalizeClass, f as unref, F as Fragment, ao as renderList, e as createBaseVNode, a as createVNode, w as withCtx, t as toDisplayString, aF as renderSlot, j as createTextVNode, o as openBlock, aU as withModifiers, aq as nextTick } from "./index-CB8xvhFr.js";
import { c as useDropZone, a as useStorage } from "./index-CMOU2boc.js";
import { s as showDirectoryDialog } from "./showDirectoryDialog-CbG0Fkj3.js";
import { b as formatFile } from "./index-guw4y6OO.js";
...
const useDrive = () => {
  ...
  const homeDrive = (changeTab) => {
    const moveToNextStep = (tab) => {
      if (tab) changeTab(tab);
      nextTick(() => driverObj.moveNext());
    };
    const moveToPreviousStep = (tab) => {
      if (tab) changeTab(tab);
    ...
    const driverObj = ke({
      ...
      steps: [
        ...
        },
        {
          ...
          popover: {
            title: "弹幕设置",
            description: "重点关注文字大小、文字不透明度和分辨率。建议开启“自适应视频分辨率”，让弹幕与视频尺寸保持一致。"
          }
        },
        {
          ...
          popover: {
            title: "FFmpeg 设置",
            description: "你可以根据你的硬件来选择对应的硬件编码来加速，整个应用中你可以将鼠标移至？图标上查看每个选项的详细说明。"
          }
        }
      ]
    });
