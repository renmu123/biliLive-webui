...
import { a as supportedVideoExtensions, b as formatFile } from "./index-guw4y6OO.js";
  ...
  setup(__props) {
    ...
    const convert = async () => {
      ...
      if (result.errors.length > 0 || result.warnings.length > 0) {
        const list = result.errors.map((item) => `${item}`).join("\n").concat(result.warnings.map((item) => `${item}`).join("\n"));
...
const VideoMerge = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-692d79d0"]]);
export {
